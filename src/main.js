...
const QUERY_PREVIEW_CHAR_LIMIT = 1_000_000;

function capQueryPreview(lines) {
  let used = 0;
  const kept = [];
  for (const line of lines || []) {
    const len = String(line).length + 1;
    if (used + len > QUERY_PREVIEW_CHAR_LIMIT) break;
    kept.push(line);
    used += len;
  }
  return {
...
async function openDocumentFile(path) {
  ...
  if (Number(metadata.size || 0) < state.largeFileWarnThreshold) {
    return invoke('read_file', { path });
  }
  return invoke('open_file_session', {
    path,
    previewBytes: state.largeFileChunkChars,
  });
}
...
async function restoreSession() {
  try {
    ...
    for (const snap of session.tabs) {
      ...
      try {
        const fileData = await openDocumentFile(snap.path);
...
function contentLinter(language) {
  return linter((view) => {
    if (view.state.doc.length >= state.largeFileWarnThreshold) return [];
...
function renderRecentFiles() {
  ...
  state.recentFiles.forEach(item => {
    ...
    el.addEventListener('click', async () => {
      ...
      try {
        const fileData = await openDocumentFile(item.path);
        createTab(fileData);
      } catch (err) {
...
function createEditorView(content, language, options = {}) {
  ...
  const lightweight = readOnly && content.length >= state.largeFileChunkChars;
  const extensions = [
    ...(lightweight ? [] : [basicSetup]),
    keymap.of([indentWithTab]),
    search(),
    wrapCompartment.of(state.wordWrap ? EditorView.lineWrapping : []),
    lineNumCompartment.of(state.lineNumbers ? lineNumbersExt() : []),
    ...(lightweight ? [] : getLinter(language)),
    ...
    ...(lightweight ? [] : getLanguageExtension(language)),
    EditorView.theme({
...
function createTab(fileData) {
  ...
  const tab = {
    ...
    progressive: false,
    fullContent: null,
    fileSessionId: fileData.session_id || null,
    loadedBytes: Number(fileData.loaded_bytes || 0),
    partial: Boolean(fileData.partial),
    loadedChars: 0,
    query: {
      text: '',
      active: false,
  ...
  if (!tab.progressive && tab.fileSessionId) {
    invoke('close_file_session', { sessionId: tab.fileSessionId }).catch(() => {});
...
async function renderJsonTree(tab, container) {
  if (tab.fileSessionId && tab.partial) {
    await renderJsonStructuralIndex(tab, container);
    return;
  }
...
async function renderJsonStructuralIndex(tab, container) {
  container.innerHTML = `<div class="json-viewer-container jv-root"></div>`;
  const host = container.querySelector('.json-viewer-container');
  if (!host) return;

  try {
    const rootNode = { id: 0, path: '$', kind: 'json', depth: 0, from: 0, to: 0, child_count: 0 };
    const rootChildren = await invoke('fetch_json_children', {
    ...
    if (state.activeTabId !== tab.id || !host.isConnected) return;
    rootNode.child_count = Number(rootChildren.total || 0);

    const statsBar = document.createElement('div');
    statsBar.className = 'jv-stats-bar';
    statsBar.innerHTML = `
      <span class="jv-stat-chip">${rootNode.child_count} root children</span>
      <span class="jv-stat-chip">Depth indexed lazily</span>
      ${rootChildren.truncated ? '<span class="jv-stat-chip">bounded preview</span>' : ''}
      ${rootChildren.error ? `<span class="jv-stat-chip">${escapeHtml(rootChildren.error)}</span>` : ''}
    `;
    host.appendChild(statsBar);
    ...
    const renderNodeRow = (node) => {
      ...
      line.className = 'jv-node';
      line.dataset.path = node.path;
      line.dataset.nodeId = String(node.id);
      line.dataset.depth = String(node.depth || 0);
      line.style.paddingLeft = `${Math.min(Number(node.depth || 0), 12) * 16}px`;

      const row = document.createElement('div');
      row.className = 'jv-line';

      const spacer = document.createElement('span');
      spacer.className = node.child_count > 0 ? 'jv-arrow expanded' : 'jv-spacer';
      spacer.textContent = node.child_count > 0 ? '▼' : '';
      if (node.child_count > 0) {
        spacer.title = 'Load indexed children';
        spacer.addEventListener('click', async (e) => {
          e.stopPropagation();
          const nodeId = Number(line.dataset.nodeId || -1);
          if (nodeId < 0 || renderedChildren.has(nodeId)) return;
          try {
            const childResult = await invoke('fetch_json_children', {
              sessionId: tab.fileSessionId,
              nodeId,
              offset: 0,
              limit: 200,
      ...
      key.textContent = node.path || '$';
      row.appendChild(key);

      const badge = document.createElement('span');
      badge.className = `jv-badge jv-badge-${node.kind}`;
      badge.textContent = node.kind;
      row.appendChild(badge);

      const size = document.createElement('span');
      size.className = 'jv-size visible';
      size.textContent = node.child_count > 0
        ? ` ${node.child_count} children · bytes ${node.from}-${node.to}`
        : ` bytes ${node.from}-${node.to}`;
    ...
    if (rootNode) {
      frag.appendChild(renderNodeRow(rootNode));
      renderedChildren.add(rootNode.id);
      for (const node of rootChildren.nodes || []) {
        frag.appendChild(renderNodeRow(node));
      }
    }
    tree.appendChild(frag);
    host.appendChild(tree);
    if (ensureQueryState(tab).active && ensureQueryState(tab).pathTokens?.length) {
      applyQueryViewEffects(tab);
    }
  } catch (err) {
    console.error('JSON index render error:', err);
    host.textContent = `Unable to index JSON preview: ${err.message}`;
  }
}
...
async function renderCsvTable(tab, container) {
  try {
    ...
    if (tab.fileSessionId && tab.partial) {
      const native = await invoke('get_csv_rows', {
        sessionId: tab.fileSessionId,
        offset: 0,
        limit: 5000,
      });
      if (state.activeTabId !== tab.id || !host.isConnected) return;
      viewer.renderParsed({
        delimiter: native.delimiter === '\\t' ? '\t' : native.delimiter,
        header: native.header || [],
        rows: native.rows || [],
        rowCount: Number(native.row_count || 0),
        colCount: Number(native.col_count || 0),
...
function applyLargeFileSafetyPolicy(tab) {
  tab.largeFileMode = tab.size >= state.largeFileWarnThreshold || tab.partial;
  tab.readOnly = tab.largeFileMode;
  tab.progressive = Boolean(tab.partial);
  tab.fullContent = null;
  tab.loadedBytes = Number(tab.loadedBytes || tab.content?.length || 0);
  tab.loadedChars = (tab.content || '').length;

}

async function loadNextChunk(tabId) {
  const tab = state.tabs.find(t => t.id === tabId);
  if (!tab || !tab.progressive) return;

  if (!tab.fileSessionId) return;

  try {
    const range = await invoke('read_file_range', {
      sessionId: tab.fileSessionId,
      offset: tab.loadedBytes || 0,
      length: state.largeFileChunkChars,
    });
    tab.content += range.content || '';
    tab.loadedBytes = (tab.loadedBytes || 0) + Number(range.loaded_bytes || 0);
    tab.loadedChars = tab.content.length;

    if (range.eof || tab.loadedBytes >= tab.size) {
      tab.partial = false;
    ...
    alert(`Could not load next chunk: ${err.message}`);
  }

  if (state.activeTabId === tab.id) showEditor(tab.id);
}

async function loadFullLargeFile(tabId) {
  const tab = state.tabs.find(t => t.id === tabId);
  if (!tab || !tab.largeFileMode) return;

  if (tab.fileSessionId && tab.partial) {
    try {
      const fileData = await invoke('read_file', { path: tab.path });
      tab.content = fileData.content;
      tab.loadedBytes = Number(fileData.loaded_bytes || fileData.size || tab.content.length);
      tab.loadedChars = tab.content.length;
      tab.partial = false;
      tab.progressive = false;
      invoke('close_file_session', { sessionId: tab.fileSessionId }).catch(() => {});
      ...
      console.error('Load full file error:', err);
      alert(`Could not load full file: ${err.message}`);
      return;
    }
  }
...
function renderLargeFileBanner(tab, container) {
  ...
  if (tab.progressive || tab.partial) {
    const loaded = Number(tab.loadedBytes || tab.loadedChars || 0);
    const percent = tab.size ? Math.min(100, Math.round((loaded / tab.size) * 100)) : 0;
    meta.textContent = `Progressive preview: ${formatSize(loaded)} / ${formatSize(tab.size)} loaded (${percent}%).`;
  } else {
  ...
  if (tab.progressive && tab.fileSessionId && (tab.loadedBytes || 0) < tab.size) {
...
async function applyLogQuery(tab, rawQuery) {
  ...
  try {
    ...
    if (tab.fileSessionId && tab.partial) {
      ...
      result = {
        error: native.error || '',
        filteredLines: native.filtered_lines || [],
        resultCount: Number(native.result_count || 0),
        totalCount: Number(native.total_count || 0),
        clauseCount: Number(native.clause_count || 0),
        termCount: Number(native.term_count || 0),
        clauses: native.clauses || [],
        truncated: Boolean(native.truncated),
      };
    } else {
      result = await workerBridge.filterLog(source, rawQuery);
    }
    // Guard: query may have changed while worker was running
    if (query.text !== rawQuery) return;

    const preview = result.error ? { content: null, truncated: false } : capQueryPreview(result.filteredLines);
    query.totalCount = result.totalCount;
    query.resultCount = result.resultCount;
    query.previewContent = preview.content;
    query.clauseCount = result.clauseCount || 0;
    query.termCount = result.termCount || 0;
    query.clauses = result.clauses || [];
    query.error = result.error || '';
    query.truncated = Boolean(result.truncated || preview.truncated);
    query.busy = false;
...
async function applyJsonQuery(tab, rawQuery) {
  ...
  if (tab.fileSessionId && tab.partial) {
    ...
    if (tokens.length === 0 && rawQuery.trim() !== '$') {
      query.active = true;
    ...
    try {
      const result = await invoke('lookup_json_path_session', {
        sessionId: tab.fileSessionId,
        rawPath: rawQuery,
        maxNodes: 100000,
        maxDepth: Math.max(16, tokens.length + 4),
        maxBytes: 64 * 1024 * 1024,
      });
      if (query.text !== rawQuery) return;
      query.resultCount = result.found ? 1 : 0;
      query.totalCount = 1;
      query.locateResult = result.found
        ? {
            from: Number(result.from || 0),
            to: Number(result.to || result.from || 0),
            valueFrom: Number(result.from || 0),
            valueTo: Number(result.to || result.from || 0),
            line: Number(result.line || 1),
            col: Number(result.col || 1),
          }
        : null;
      query.error = result.error || (result.found ? '' : `Path not found: ${rawQuery}`);
      if (!result.found && result.truncated && !query.error) {
        query.error = `Path not found in indexed preview: ${rawQuery}`;
      }
      query.truncated = Boolean(result.truncated);
      query.busy = false;
...
function highlightJsonPathInTree(pathTokens) {
  ...
  root.querySelectorAll('.jv-node.query-highlight').forEach(el => el.classList.remove('query-highlight'));

  const indexedPath = '$' + pathTokens.map(token => (/^\d+$/.test(String(token)) ? `[${token}]` : `.${token}`)).join('');
  const indexedTarget = [...root.querySelectorAll('.jv-node')].find(node => node.dataset.path === indexedPath);
  if (indexedTarget) {
    indexedTarget.classList.add('query-highlight');
    indexedTarget.scrollIntoView({ block: 'center', behavior: 'smooth' });
    return;
  }
...
function updateQueryBar(tab) {
  ...
  if (query.error) {
    ...
    if (jsonMode && query.resultCount > 0 && Array.isArray(query.pathTokens) && query.pathTokens.length > 0) {
      ...
      const truncated = query.truncated ? ', preview capped' : '';
      if (query.totalCount !== null) meta.textContent = `${query.resultCount} / ${query.totalCount} (${suffix}${truncated})`;
...
async function openFile() {
  try {
    ...
    if (existing) { switchToTab(existing.id); return; }
    const fileData = await openDocumentFile(filePath);
    createTab(fileData);
  ...
  entries.forEach(entry => {
    ...
    if (entry.is_dir) {
      ...
      item.addEventListener('click', async (e) => {
        ...
        if (existing) { switchToTab(existing.id); return; }
        try { createTab(await openDocumentFile(entry.path)); }
...
function setupDragDrop() {
  ...
  body.addEventListener('drop', async (e) => {
    ...
    for (const file of files) {
      ...
      try {
        await invoke('approve_path', { path }).catch(() => {});
        createTab(await openDocumentFile(path));
      }
...
async function executeExtensionCommand(command) {
  ...
  if (command.type === 'open_file') {
    ...
    try {
      ...
      const data = await openDocumentFile(target);
      createTab(data);
    } catch (err) {
  ...
  results.forEach((r, i) => {
    ...
    item.addEventListener('click', async () => {
      ...
      if (r.type === 'tab') {
        ...
        if (existing) { switchToTab(existing.id); return; }
        try { createTab(await openDocumentFile(r.path)); }
...
function disposeTabHeavyState(tab) {
  if (!tab) return;
  const query = ensureQueryState(tab);
  if (query.previewContent && query.previewContent.length > state.largeFileChunkChars) {
    query.previewContent = query.previewContent.slice(0, state.largeFileChunkChars);
...
function compactInactiveLargeTabs() {
  ...
  const active = state.tabs.find(t => t.id === state.activeTabId);
  const activeSessionIds = active?.fileSessionId ? [active.fileSessionId] : [];
  invoke('compact_session_caches', { activeSessionIds }).catch(() => {});
}

let _memoryCleanupTimer = null;
function scheduleMemoryCleanup() {
  clearTimeout(_memoryCleanupTimer);
  _memoryCleanupTimer = setTimeout(compactInactiveLargeTabs, 1000);
}
